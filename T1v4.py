...
# Set min and max breakdown time for scaling
min_time = 24  # Lower bound
max_time = 48  # Upper bound
...
def train_model(training_file_path):
    ...
    def preprocess_data(X, y):
        mask = (y >= min_time) & (y <= max_time)  # Filtering y between 24 and 48
        X_filtered = X[mask]
        y_filtered = y[mask]

        # Scale y values to range 0-1 for sigmoid activation
        y_filtered_scaled = (y_filtered - min_time) / (max_time - min_time)

        X_train, X_val, y_train, y_val = train_test_split(X_filtered, y_filtered_scaled, test_size=0.01, random_state=42)
        scaler = StandardScaler()
        X_train_scaled = scaler.fit_transform(X_train)
        X_val_scaled = scaler.transform(X_val)
        joblib.dump(scaler, os.path.join(model_folder_path, 'scalerfinpv1234.pkl'))
    ...
    def build_model(input_shape):
        model = Sequential([
            ...
            Dense(1, activation='sigmoid')  # Ensures output is between 0 and 1
        ])
    ...
    model.save(os.path.join(model_folder_path, 'trained_modelFINpv1234.h5'))
...
def predict_time(test_file_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, 'scalerfinpv1234.pkl'))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict_time_to_breakdown(X_test_scaled):
        model = load_model(os.path.join(model_folder_path, 'trained_modelFINpv1234.h5'))
        scaled_predictions = model.predict(X_test_scaled)

        # Scale back to 24-48 range
        final_predictions = min_time + (max_time - min_time) * scaled_predictions
        return final_predictions.flatten()  # Convert to 1D array
    ...
    try:
        ...
        # Ensure predictions stay within range
        predictions = np.clip(predictions, min_time, max_time)

        # Return the final breakdown time
        return f"Breakdown might occur in approximately w.r.t 6 AM yesterday: \n{predictions.mean():.2f} hours"
